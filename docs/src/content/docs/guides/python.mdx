...
If your `mise.toml` file reference a `venv` like:
...
The VSCode python extension does not use the `python.defaultInterpreterPath` setting if an interpret was previously set (see [this document](https://github.com/microsoft/vscode-python/wiki/Setting-descriptions) for more details.

For this reason, you might need to have to manually switch the python interpreter if the wrong one is selected.
