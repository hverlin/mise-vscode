...
export function extractToolNamesFromLine(
	...
	// Section headers: `[tools.node]`, `[tools."npm:prettier"]`, ...
	// Sub-tables like `[tools."http:my-tool".platforms]` do not declare a new tool.
	const header = parseToolsSectionHeader(trimmed);
	if (header) {
		if (header.isSubTable || (word && !header.toolName.includes(word))) {
			return [];
		}
	...
	// Key after the dot can be bare or quoted (single/double), and may be dotted
	// (e.g. `tools.node.version = "22"`).
	const dotMatch = trimmed.match(
		/^\s*tools\s*\.\s*((?:["'][^"']+["'])|(?:[^\s=]+))\s*=/,
	);
	if (dotMatch?.[1]) {
		const rawKey = splitDottedToolKey(dotMatch[1]).toolName;
	...
	// Key can be bare (no spaces), fully quoted, or dotted
	// (e.g. `node.version = "22"` or `"ubi:sharkdp/fd".version = "latest"`).
	const blockMatch = trimmed.match(/^((?:["'][^"']*["'])|(?:[^\s=]+))\s*=/);
	if (blockMatch?.[1]) {
		const rawKey = splitDottedToolKey(blockMatch[1]).toolName;
		if (word && !rawKey.includes(word)) {
			return [];
		}
		results.push(rawKey);
...
export function extractToolVersionFromLine(
	...
	const patterns = [
		...
		// `node.version = "20"` or `tools."ubi:sharkdp/fd".version = "latest"`
		new RegExp(
			`(?:tools\\.)?['"]?${escapedToolName}['"]?\\.version\\s*=\\s*['"]([^'"]+)['"]`,
		),
		// `node = ["20", "22"]` → first entry
		new RegExp(
			`(?:tools\\.)?['"]?${escapedToolName}['"]?\\s*=\\s*\\[\\s*['"]([^'"]+)['"]`,
		),
	];
...
/**
 * Extracts the requested version from the body of a `[tools.<name>]` section:
 *
 * ```toml
 * [tools.node]
 * version = "22"
 ...
 * ```
 *
 * Supports both string values and arrays of versions (first entry is returned).
 */
export function extractToolVersionFromSection(
	document: vscode.TextDocument,
	headerLine: number,
): string | undefined {
